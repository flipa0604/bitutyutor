...
_SURVEY_FOOTER = "\n\n🗂 " + texts.SURVEY_SHORT_LABELS["basic"]


def basic_list(group: str, n: int, tutor: str = "") -> str:
    """The student list of the basic questionnaire, as the screen renders it."""
    return texts.STUDENT_LIST_TITLE.format(group=group, n=n, tutor=tutor) + _SURVEY_FOOTER


def basic_empty(group: str, tutor: str = "") -> str:
    return texts.STUDENT_LIST_EMPTY.format(group=group, tutor=tutor) + _SURVEY_FOOTER
...
async def test_tutor_reaches_students_from_panel_command_and_group_card(h: Harness) -> None:
    ...
    assert f"stu:svs:{group.id}:t:basic" in inline_buttons(h.last_message(TUTOR).reply_markup)

    await h.feed(callback_update(tutor_user, f"tut:view:{group.id}:"))
    card_kb = inline_buttons(h.last_shown(TUTOR).reply_markup)
    assert card_kb[f"stu:svs:{group.id}:t:basic"] == texts.BTN_GROUP_STUDENTS

    await h.feed(callback_update(tutor_user, f"stu:list:{group.id}:t:basic"))
    listing = h.last_shown(TUTOR)
    assert listing.text == basic_list("DI-21", 1)
    assert inline_buttons(listing.reply_markup) == {
        f"stu:view:{student.id}:t:basic": "Zokirov Vali",
        f"stu:svs:{group.id}:t:basic": texts.BTN_BACK,
    }

    await h.feed(callback_update(tutor_user, f"stu:view:{student.id}:t:basic"))
    ...
    assert inline_buttons(card.reply_markup) == {
        f"stu:msg:{student.id}:t:basic": texts.BTN_SEND_MESSAGE,
        f"stu:del:{student.id}:t:basic": texts.BTN_DELETE,
        f"stu:list:{group.id}:t:basic": texts.BTN_BACK,
    }

    # an empty group says so and still offers the way back
    empty = await h.db.add_group(tutor.id, "DI-22")
    await h.feed(callback_update(tutor_user, f"stu:list:{empty.id}:t:basic"))
    shown = h.last_shown(TUTOR)
    assert shown.text == basic_empty("DI-22")
    assert inline_buttons(shown.reply_markup) == {f"stu:svs:{empty.id}:t:basic": texts.BTN_BACK}
...
async def test_tutor_messages_a_student(h: Harness) -> None:
    ...
    await h.feed(callback_update(tutor_user, f"stu:view:{student.id}:t:basic"))
    await h.feed(callback_update(tutor_user, f"stu:msg:{student.id}:t:basic"))
    ...
    assert f"stu:msg:{student.id}:t:basic" in inline_buttons(sent[-1].reply_markup)
...
async def test_message_and_names_are_html_escaped(h: Harness) -> None:
    ...
    await h.feed(callback_update(tutor_user, f"stu:del:{student.id}:t:basic"))
    assert h.last_shown(TUTOR).text == texts.STUDENT_DELETE_CONFIRM.format(name="Ali &lt;b&gt;Valiyev", group="DI-21", survey=texts.SURVEY_SHORT_LABELS["basic"])
    assert_all_callbacks_answered(h)
...
async def test_cancel_commands_and_menu_buttons_preempt_the_prompt(h: Harness) -> None:
    ...
    await h.feed(callback_update(tutor_user, f"stu:msg:{student.id}:t:basic"), text_update(tutor_user, "/cancel"))
    assert h.last_text(TUTOR) == texts.CANCELLED and await h.state_of(TUTOR) is None
    await h.feed(callback_update(tutor_user, f"stu:msg:{student.id}:t:basic"), text_update(tutor_user, texts.BTN_CANCEL))
    assert h.last_text(TUTOR) == texts.CANCELLED and await h.state_of(TUTOR) is None
    await h.feed(callback_update(tutor_user, f"stu:msg:{student.id}:t:basic"), text_update(tutor_user, "/tutor"))
    assert texts.TUTOR_PANEL.format(name="Karimov Aziz") in h.last_text(TUTOR) and await h.state_of(TUTOR) is None
    await h.feed(callback_update(tutor_user, f"stu:msg:{student.id}:t:basic"), text_update(tutor_user, texts.BTN_REGISTER))
    assert await h.state_of(TUTOR) is None  # the student router got it: the survey picker has no state
    assert "Qaysi anketani" in h.last_text(TUTOR)
...
async def test_delivery_failures_are_reported_honestly(h: Harness, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    await h.feed(callback_update(tutor_user, f"stu:msg:{student.id}:t:basic"), text_update(tutor_user, "Salom"))
    ...
    failure.update(exc=TelegramForbiddenError, message="Forbidden: bot was blocked by the user")
    await h.feed(
        callback_update(tutor_user, f"stu:del:{student.id}:t:basic"),
        callback_update(tutor_user, f"stu:delok:{student.id}:t:basic"),
        callback_update(tutor_user, f"stu:bye_y:{student.id}:t:basic"),
...
async def test_delete_student_then_send_a_farewell_message(h: Harness) -> None:
    ...
    await h.feed(callback_update(tutor_user, f"stu:del:{sid}:t:basic"))
    confirm = h.last_shown(TUTOR)
    assert confirm.text == texts.STUDENT_DELETE_CONFIRM.format(name="Zokirov Vali", group="DI-21", survey=texts.SURVEY_SHORT_LABELS["basic"])
    assert inline_buttons(confirm.reply_markup) == {
        f"stu:delok:{sid}:t:basic": texts.BTN_YES_DELETE,
        f"stu:view:{sid}:t:basic": texts.BTN_NO,
    }
    await h.feed(callback_update(tutor_user, f"stu:view:{sid}:t:basic"))  # changed my mind
    assert h.last_shown(TUTOR).text.startswith(texts.STUDENT_CARD_TITLE)
    assert await h.db.get_student_by_telegram_id(STUDENT) is not None

    await h.feed(callback_update(tutor_user, f"stu:del:{sid}:t:basic"), callback_update(tutor_user, f"stu:delok:{sid}:t:basic"))
    assert await h.db.get_student_by_telegram_id(STUDENT) is None
    ...
    assert inline_buttons(question.reply_markup) == {
        f"stu:bye_y:{sid}:t:basic": texts.BTN_YES_SEND_MESSAGE,
        f"stu:bye_n:{sid}:t:basic": texts.BTN_NO,
    }
    ...
    await h.feed(callback_update(tutor_user, f"stu:bye_y:{sid}:t:basic"))
    ...
    await h.feed(callback_update(tutor_user, f"stu:bye_y:{sid}:t:basic"))  # ✉️ Ha tapped again: just a toast
    ...
    assert reply_button_texts(sent[-2].reply_markup) == TUTOR_MENU
    assert sent[-1].text == basic_empty("DI-21")
    assert await h.state_of(TUTOR) is None
    ...
    await h.feed(callback_update(tutor_user, f"stu:bye_n:{sid}:t:basic"))
    assert h.answers()[-1].text == texts.FAREWELL_STALE and h.answers()[-1].show_alert

    # the student is free to fill the questionnaire in again from scratch
    ...
    await h.feed(callback_update(student_user, "sv:fill:basic"))
...
async def test_delete_student_without_a_message(h: Harness) -> None:
    ...
    assert inline_buttons(shown.reply_markup) == {
        f"stu:view:{second.id}:t:basic": "Aliyev Olim",
        f"stu:svs:{group.id}:t:basic": texts.BTN_BACK,
    }
    assert h.messages(STUDENT) == []
    assert await h.db.get_student_by_telegram_id(STUDENT) is None
...
async def test_cancelling_at_the_farewell_question_keeps_the_deletion(h: Harness) -> None:
    ...
    await h.feed(callback_update(tutor_user, f"stu:delok:{sid}:t:basic"), text_update(tutor_user, texts.BTN_CANCEL))
    assert h.last_text(TUTOR) == texts.CANCELLED and await h.state_of(TUTOR) is None
    assert await h.db.get_student_by_telegram_id(STUDENT) is None
    await h.feed(callback_update(tutor_user, f"stu:bye_y:{sid}:t:basic"))
...
async def test_stale_farewell_button_from_an_earlier_deletion_is_refused(h: Harness) -> None:
    ...
    await h.feed(callback_update(tutor_user, f"stu:delok:{first.id}:t:basic"))  # question #1, left unanswered
    await h.feed(
        callback_update(tutor_user, f"stu:list:{group.id}:t:basic"),
        callback_update(tutor_user, f"stu:delok:{second.id}:t:basic"),  # question #2
    )
    assert await h.state_of(TUTOR) == StudentManage.farewell.state

    await h.feed(callback_update(tutor_user, f"stu:bye_y:{first.id}:t:basic"))  # the old ✉️ Ha
    assert h.answers()[-1].text == texts.FAREWELL_STALE and h.answers()[-1].show_alert
    assert await h.state_of(TUTOR) == StudentManage.farewell.state  # #2 is still open
    await h.feed(callback_update(tutor_user, f"stu:bye_y:{second.id}:t:basic"), text_update(tutor_user, "Xayr"))
    assert h.messages(STUDENT) == []
...
async def test_concurrent_double_tap_deletes_once(h: Harness) -> None:
    ...
    await h.feed_concurrently(
        callback_update(tutor_user, f"stu:delok:{sid}:t:basic"), callback_update(tutor_user, f"stu:delok:{sid}:t:basic")
    )
...
async def test_group_gone_between_screens(h: Harness) -> None:
    ...
    await h.feed(
        callback_update(tutor_user, f"stu:view:{student.id}:t:basic"),
        callback_update(tutor_user, f"stu:delok:{student.id}:t:basic"),
        callback_update(tutor_user, f"stu:list:{group.id}:t:basic"),
    )
...
async def test_other_tutor_and_strangers_are_denied(h: Harness) -> None:
    ...
    for data in (
        f"stu:list:{group.id}:t:basic",
        f"stu:view:{sid}:t:basic",
        f"stu:msg:{sid}:t:basic",
        f"stu:del:{sid}:t:basic",
        f"stu:delok:{sid}:t:basic",
        f"stu:msg:{sid}:a:basic",  # forging the admin mode does not help
        f"stu:delok:{sid}:a:basic",
    ):
    ...
    await h.feed(callback_update(stranger, f"stu:list:{group.id}:t:basic"), callback_update(stranger, f"stu:delok:{sid}:a:basic"))
...
async def test_superadmin_manages_any_students_through_the_tutor_card(h: Harness) -> None:
    ...
    await h.feed(callback_update(admin, f"stu:list:{group.id}:a:basic"))
    listing = h.last_shown(SUPERADMIN)
    tutor_line = texts.STUDENT_LIST_TUTOR_LINE.format(tutor="Karimov Aziz")
    assert listing.text == basic_list("DI-21", 1, tutor_line)
    assert inline_buttons(listing.reply_markup) == {
        f"stu:view:{sid}:a:basic": "Zokirov Vali",
        f"stu:svs:{group.id}:a:basic": texts.BTN_BACK,
    }

    await h.feed(callback_update(admin, f"stu:view:{sid}:a:basic"))
    assert inline_buttons(h.last_shown(SUPERADMIN).reply_markup) == {
        f"stu:msg:{sid}:a:basic": texts.BTN_SEND_MESSAGE,
        f"stu:del:{sid}:a:basic": texts.BTN_DELETE,
        f"stu:list:{group.id}:a:basic": texts.BTN_BACK,
    }
    await h.feed(callback_update(admin, f"stu:msg:{sid}:a:basic"), text_update(admin, "Diqqat!"))
    ...
    await h.feed(
        callback_update(admin, f"stu:del:{sid}:a:basic"),
        callback_update(admin, f"stu:delok:{sid}:a:basic"),
        callback_update(admin, f"stu:bye_y:{sid}:a:basic"),
    ...
    assert sent[-2].text == texts.STUDENT_DELETED_MESSAGE_SENT.format(name="Zokirov Vali")
    assert sent[-1].text == basic_empty("DI-21", tutor_line)
    assert inline_buttons(sent[-1].reply_markup) == {f"stu:svs:{group.id}:a:basic": texts.BTN_BACK}
    assert_all_callbacks_answered(h)
...
async def test_superadmin_who_is_a_tutor_signs_own_students_by_name(h: Harness) -> None:
    ...
    await h.feed(callback_update(both_user, f"stu:msg:{own.id}:a:basic"), text_update(both_user, "Salom"))
    ...
    await h.feed(callback_update(both_user, f"stu:msg:{student.id}:t:basic"), text_update(both_user, "Salom"))
    ...
    await h.feed(callback_update(both_user, f"stu:view:{student.id}:t:basic"))
    assert f"stu:list:{group.id}:a:basic" in inline_buttons(h.last_shown(BOTH).reply_markup)
    assert_all_callbacks_answered(h)
...
async def test_tutor_delete_is_scoped_to_their_own_students_even_in_a_race(
    ...
    await h.feed(callback_update(tutor_user, f"stu:delok:{student.id}:t:basic"))
...
async def test_superadmin_tutor_deletes_another_tutors_student(h: Harness) -> None:
    ...
    await h.feed(callback_update(both_user, f"stu:delok:{student.id}:a:basic"))
    assert h.answers()[-1].text == texts.STUDENT_DELETED_TOAST
    assert await h.db.get_student_by_telegram_id(STUDENT) is None
    assert_all_callbacks_answered(h)
...
async def test_farewell_text_is_validated_like_a_message(h: Harness) -> None:
    ...
    tutor_user = make_user(TUTOR)
    sid = student.id
    await h.feed(callback_update(tutor_user, f"stu:delok:{sid}:t:basic"), callback_update(tutor_user, f"stu:bye_y:{sid}:t:basic"))
    assert await h.state_of(TUTOR) == StudentManage.farewell_text.state
...
async def test_saying_no_after_yes_drops_the_open_prompt(h: Harness) -> None:
    ...
    await h.feed(callback_update(tutor_user, f"stu:bye_n:{sid}:t:basic"))  # the question was still on screen
    assert await h.state_of(TUTOR) is None
...
async def test_stale_farewell_tap_leaves_the_new_list_keyboard_alone(h: Harness) -> None:
    ...
    tutor_user = make_user(TUTOR)
    sid = student.id
    await h.feed(callback_update(tutor_user, f"stu:delok:{sid}:t:basic"))
    await h.feed(callback_update(tutor_user, f"stu:bye_n:{sid}:t:basic"), callback_update(tutor_user, f"stu:bye_n:{sid}:t:basic"))
    assert h.answers()[-1].text == texts.FAREWELL_STALE
    assert h.of("EditMessageReplyMarkup", TUTOR) == []  # the freshly drawn student list keeps its buttons
    second = await h.db.get_student_by_telegram_id(SECOND_STUDENT)
    assert second is not None
    assert f"stu:view:{second.id}:t:basic" in inline_buttons(h.last_shown(TUTOR).reply_markup)
    assert_all_callbacks_answered(h)
...
async def test_inline_navigation_abandons_an_open_prompt(h: Harness) -> None:
    tutor, group, student = await seed(h)
    tutor_user = make_user(TUTOR)
    for data in (f"stu:list:{group.id}:t:basic", f"stu:view:{student.id}:t:basic", f"stu:del:{student.id}:t:basic"):
        await h.feed(callback_update(tutor_user, f"stu:msg:{student.id}:t:basic"), callback_update(tutor_user, data))
...
async def test_a_tutor_cannot_ask_for_admin_style_navigation(h: Harness) -> None:
    tutor, group, student = await seed(h)
    tutor_user = make_user(TUTOR)
    await h.feed(callback_update(tutor_user, f"stu:view:{student.id}:a:basic"))
    assert f"stu:list:{group.id}:t:basic" in inline_buttons(h.last_shown(TUTOR).reply_markup)
    await h.feed(callback_update(tutor_user, f"stu:list:{group.id}:a:basic"))
    assert f"stu:svs:{group.id}:t:basic" in inline_buttons(h.last_shown(TUTOR).reply_markup)
    assert h.last_shown(TUTOR).text == basic_list("DI-21", 1)
    assert_all_callbacks_answered(h)
...
async def test_every_screen_escapes_group_and_tutor_names(h: Harness) -> None:
    ...
    await h.feed(callback_update(tutor_user, f"stu:list:{group.id}:t:basic"))
    assert h.last_shown(TUTOR).text == basic_list(esc_group, 1)
    await h.feed(callback_update(admin, f"adm:groups:{tutor.id}"))
    ...
    assert h.last_shown(SUPERADMIN).text == basic_list(esc_group, 1, tutor_line)

    await h.feed(callback_update(tutor_user, f"stu:delok:{student.id}:t:basic"))
    assert h.last_shown(TUTOR).text == texts.STUDENT_DELETED_ASK_MESSAGE.format(name=esc_name)
    await h.feed(callback_update(tutor_user, f"stu:bye_y:{student.id}:t:basic"))
    ...
    assert h.messages(TUTOR)[-1].text == basic_empty(esc_group)
    assert_all_callbacks_answered(h)
