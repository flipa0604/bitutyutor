...
async def _load_student(callback: CallbackQuery, db: Database, actor: Actor, student_id: int) -> Student | None:
    ...
    if student is None:
        ...
        return None
    if not actor.may_manage(student.tutor_id):
        log.warning("User %s denied on student %s owned by tutor %s", actor.user_id, student.id, student.tutor_id)
        await callback.answer(texts.NO_PERMISSION, show_alert=True)
        return None
    return student
...
async def _student_list(db: Database, group: Group, actor: Actor) -> tuple[str, InlineKeyboardMarkup]:
    mode = actor.mode_for(group.tutor_id)
    students = await db.list_students(group_id=group.id)
    tutor_line = ""
    if mode == VIA_ADMIN:  # group names are unique per tutor only, so say whose group this is
        owner = await db.get_tutor(group.tutor_id)
        if owner is not None:
            tutor_line = texts.STUDENT_LIST_TUTOR_LINE.format(tutor=hesc(owner.name))
    if students:
        text = texts.STUDENT_LIST_TITLE.format(group=hesc(group.name), n=len(students), tutor=tutor_line)
    else:
        text = texts.STUDENT_LIST_EMPTY.format(group=hesc(group.name), tutor=tutor_line)
    return text, student_list_kb(students, group, mode)


def _card(student: Student) -> str:
    return texts.student_card(texts.STUDENT_CARD_TITLE, student)
...
async def cb_list(
    ...
    text, kb = await _student_list(db, group, actor)
...
async def cb_view(
    ...
    actor = _actor(callback.from_user.id, is_admin, tutor, callback_data.via)
    student = await _load_student(callback, db, actor, callback_data.id)
    if student is None:
        return
    await state.clear()
    await callback.answer()
    await edit_or_send(callback, bot, _card(student), student_manage_kb(student, actor.mode_for(student.tutor_id)))
...
async def cb_message(
    ...
    await state.set_data({"student_id": student.id, "via": actor.via})
    await callback.answer()
...
async def message_text(
    ...
    student = await db.get_student(int(data.get("student_id") or 0))
    ...
    await message.answer(_card(student), reply_markup=student_manage_kb(student, mode))
...
async def cb_delete(
    ...
    await edit_or_send(
        callback,
        bot,
        texts.STUDENT_DELETE_CONFIRM.format(name=hesc(student.full_name), group=hesc(student.group_name)),
        student_confirm_delete_kb(student, actor.mode_for(student.tutor_id)),
    )
...
async def cb_confirm_delete(
    ...
    if not await db.delete_student(student.id, tutor_id=scope):  # gone already (double tap) or moved
        await callback.answer(texts.STUDENT_NOT_FOUND, show_alert=True)
        return
    mode = actor.mode_for(student.tutor_id)
    log.info(
    ...
    await edit_or_send(
        ...
        student_farewell_kb(student.id, mode),
    )


async def _show_list_after(
    callback: CallbackQuery, bot: Bot, db: Database, actor: Actor, group_id: int, lead: str
) -> None:
...
async def cb_farewell(
    ...
    await _show_list_after(
        callback, bot, db, actor, int(data.get("group_id") or 0), texts.STUDENT_DELETED_NO_MESSAGE.format(name=name)
    )
...
async def farewell_text(
    ...
    if group is not None and actor.may_manage(group.tutor_id):
        list_text, kb = await _student_list(db, group, actor)
